var __JS_TEST = false;
// Time-stamp: <2011-11-23 16:52:01 Administrator> 
// version: http://jquery-easyui.googlecode.com/svn/r525
/**
 * jQuery EasyUI 1.2.5
 * 
 ...
 */
(function($) {
	//if(__JS_TEST)Test.run('draggable');
	var standby = false;
	function drag(e) {
 ...
 */
(function($) {
//if(__JS_TEST)Test.run('droppable');
 ...
 */
(function($) {
//if(__JS_TEST)Test.run('resizable');
	var standby = false;
	$.fn.resizable = function(options, param) {
		...
		function resize(e) {
			...
			if (resizeData.dir.indexOf('w') != -1) {
				resizeData.width = resizeData.startWidth - e.pageX
						+ resizeData.startX;
				if (resizeData.width >= options.minWidth
						&& resizeData.width <= options.maxWidth) {
		...
		return this.each(function() {
			...
			$(this).bind('mousemove.resizable', {
				...
				if (dir == '') {
					$(e.data.target).css('cursor', 'default');
				} else {
 ...
 */
(function($) {
//if(__JS_TEST)Test.run('linkbutton');
 ...
 */
(function($) {
//if(__JS_TEST)Test.run('pagination');
	function buildToolbar(target) {
		...
		if (opts.showPageList) {
			...
			for ( var i = 0; i < opts.pageList.length; i++) {
				var pageItem = $('<option></option>').text(opts.pageList[i])
						.appendTo(ps);
		...
		pager.find('a[icon=pagination-load]').unbind('.pagination').bind(
				...
				function() {
					if (opts.onBeforeRefresh.call(target, opts.pageNumber,
						...
						opts.onRefresh.call(target, opts.pageNumber,
								opts.pageSize);
					}
				});
	...
	function showInfo(target) {
		...
		pinfo = pinfo.replace(/{from}/, opts.pageSize * (opts.pageNumber - 1)
				+ 1);
		pinfo = pinfo.replace(/{to}/, Math.min(opts.pageSize
				* (opts.pageNumber), opts.total));
		pinfo = pinfo.replace(/{total}/, opts.total);
		$(target).find('.pagination-info').html(pinfo);
		$('a[icon=pagination-first],a[icon=pagination-prev]', target)
				.linkbutton({
					disabled : (opts.pageNumber == 1)
				});
		$('a[icon=pagination-next],a[icon=pagination-last]', target)
				.linkbutton({
...
//if(__JS_TEST)Test.run('tree');
	...
	function bindTreeEvents(target) {
		...
		$('span.tree-checkbox', tree).unbind('.tree').bind(
				'click.tree',
				function() {
					var node = $(this).parent();
					setChecked(target, node[0], !$(this).hasClass(
							'tree-checkbox1'));
					return false;
				}).bind('mousedown.tree', function() {
			return false;
	...
	function loadData(target, ul, data, isAppend) {
		...
		function appendNodes(ul, children, depth) {
			for ( var i = 0; i < children.length; i++) {
				...
				$('<span class="tree-title"></span>').html(item.text).appendTo(
						node);
				if (opts.checkbox) {
					if (opts.onlyLeafCheck) {
						if (item.state == 'open'
	...
	function expand(target, nodeEl, callBack) {
		...
		if (ul.length) {
			...
			var newNode = $('<ul style="display:none"></ul>').insertAfter(
					nodeEl);
	...
	function getNode(target, nodeEl) {
		var node = $.extend({}, $.data(nodeEl, 'tree-node'), {
			target : nodeEl,
			checked : $(nodeEl).find('.tree-checkbox').hasClass(
					'tree-checkbox1')
		});
		if (!isLeaf(target, nodeEl)) {
 ...
 */
(function($) {
//if(__JS_TEST)Test.run('progressbar');
	function init(target) {
...
//if(__JS_TEST)Test.run('window');
	...
	function init(target) {
		...
		if (state.options.modal == true) {
			state.mask = $('<div class="window-mask"></div>').insertAfter(
					state.window);
			state.mask.css({
				width : (state.options.inline ? state.mask.parent().width()
						: getPageArea().width),
				height : (state.options.inline ? state.mask.parent().height()
						: getPageArea().height),
				display : 'none'
			});
		}
		...
		if (state.options.shadow == true) {
			state.shadow = $('<div class="window-shadow"></div>').insertAfter(
					state.window);
			state.shadow.css({
				display : 'none'
			});
		...
		if (state.options.left == null) {
			...
			if (state.options.inline) {
				...
				state.options.left = (parent.width() - width) / 2
						+ parent.scrollLeft();
			} else {
		...
		if (state.options.top == null) {
			...
			if (state.options.inline) {
				var parent = state.window.parent();
				state.options.top = (parent.height() - height) / 2
						+ parent.scrollTop();
			} else {
						...
						state.proxy
								.css({
									...
									width : ($.boxModel == true ? (e.data.width - (state.proxy
											.outerWidth() - state.proxy.width()))
											: e.data.width),
									height : ($.boxModel == true ? (e.data.height - (state.proxy
											.outerHeight() - state.proxy
											.height()))
											: e.data.height)
								});
				...
				.extend(
						...
						$.fn.panel.parseOptions(target),
						{
							...
							shadow : (t.attr('shadow') ? t.attr('shadow') == 'true'
									: undefined),
							noproxy : (t.attr('noproxy') ? t.attr('noproxy') == 'true'
									: undefined),
 ...
 */
(function($) {
//if(__JS_TEST)Test.run('messager');
	...
	$.messager = {
		...
		prompt : function(title, msg, fn, val) {
			var content = '<div class="messager-icon messager-question"></div>'
					+ '<div>' + msg + '</div>' + '<br/>'
			...
			if (typeof val != 'undefined') {
				$('.messager-input', win).val(val);
			}
			$('.messager-input', win).focus();
		},
 ...
 */
(function($) {
//if(__JS_TEST)Test.run('accordion');
	function setSize(container) {
		...
		if (opts.height > 0) {
			...
			var headerHeight = panels.length ? panels[0].panel('header').css(
					'height', null).outerHeight() : 'auto';
			var panelHeight = cc.height() - (panels.length - 1) * headerHeight;
		}
		for ( var i = 0; i < panels.length; i++) {
			var panel = panels[i];
			var header = panel.panel('header');
			header.height($.boxModel == true ? (headerHeight - (header
					.outerHeight() - header.height())) : headerHeight);
			panel.panel('resize', {
				width : cc.width(),
				height : panelHeight
			});
		...
		pp.panel($.extend({}, options, {
			...
			onBeforeExpand : function() {
				...
				if (currentPanel) {
					...
					// www
					var _opts = $.data(container, 'accordion').options;
					if (!_opts.showAll) {
				...
				header.find('div.accordion-collapse').removeClass(
						'accordion-expand');
			},
			onExpand : function() {
				var opts = $.data(container, 'accordion').options;
			...
			onBeforeCollapse : function() {
				var header = pp.panel('header');
				header.removeClass('accordion-header-selected');
				header.find('div.accordion-collapse').addClass(
						'accordion-expand');
			}
		}));
	...
	function select(container, title) {
		...
		if (panel) {
			...
			if (currentPanel) {
				currentPanel.panel('header').addClass(
						'accordion-header-selected');
				opts.onSelect.call(container,
						currentPanel.panel('options').title);
			}
		}
	};
	...
	$.fn.accordion = function(options, param) {
		...
		return this.each(function() {
			...
			if (state) {
				...
				// www
				if (opts.showAll) {
					$(this).children('div').attr("selected", true);
				}
	...
	$.fn.accordion.parseOptions = function(target) {
		...
		return {
			...
			animate : (t.attr('animate') ? t.attr('animate') == 'true'
					: undefined),
			showAll : (t.attr('showAll') ? t.attr('showAll') == 'true'
					: undefined)
		};
	};
...
//if(__JS_TEST)Test.run('tabs');
	...
	function setSize(container) {
		...
		if (!isNaN(height)) {
			if ($.boxModel == true) {
				...
				panels.css('height',
						(height - header.outerHeight() - borderHeight)
								|| 'auto');
			} else {
	...
	function closeTab(container, title) {
		...
		tab = getTab(container, title, true);
	...
	function selectTab(container, title) {
		...
		if (left < 0 || right > wrap.innerWidth()) {
			var pos = Math.min(leftPos - (wrap.width() - tab.width()) / 2,
					getMaxScrollWidth(container));
			wrap.animate({
				scrollLeft : pos
			}, opts.scrollDuration);
...
//if(__JS_TEST)Test.run('layout');
	...
	function init(container) {
		...
		function createPanel(dir) {
			var pp = $('>div[region=' + dir + ']', container).addClass(
					'layout-body');
			var toolCls = null;
			if (dir == 'north') {
 ...
 */
(function($) {
//if(__JS_TEST)Test.run('menubutton');
	function init(target) {
 ...
 */
(function($) {
//if(__JS_TEST)Test.run('splitbutton');
	function init(target) {
 ...
 * Copyright 2010 stworthy [ stworthy@gmail.com ]
 * 
 * Dependencies: menu menubutton
 * 
 */
(function($) {
//if(__JS_TEST)Test.run('searchbox');
	function createBox(target) {
		$(target).hide();
		var searchBox = $('<span class="searchbox"></span>')
				.insertAfter(target);
	...
	function handleMenu(target) {
		...
		if (opts.menu) {
			...
			var item = searchBox.menu.menu('getItem', searchBox.menu
			...
			$(this).removeClass('searchbox-prompt');
				...
				function(e) {
					if (e.keyCode == 13) {
						e.preventDefault();
						opts.value = $(this).val();
						opts.searcher.call(target, opts.value, searchText
								.attr('name'));
						return false;
					}
				});
 ...
 */
(function($) {
//if(__JS_TEST)Test.run('validatebox');
	function init(target) {
	...
	function showTip(target) {
		...
		if (!tip) {
			tip = $(
					'<div class="validatebox-tip" id="' + owner + '">'
 ...
 */
(function($) {
//if(__JS_TEST)Test.run('numberbox');
	function init(target) {
	...
	function initChange(target) {
		...
		fix(target, opts.parser.call(target, opts.value));
		opts.onChange = fn;
	};
	function getValue(target) {
		return $.data(target, 'numberbox').field.val();
	};
	function fix(target, val) {
		...
		if (oldVal != val) {
			opts.onChange.call(target, val, oldVal);
		}
	};
	function bindEvents(target) {
		var opts = $.data(target, "numberbox").options;
 ...
 */
(function($) {
//if(__JS_TEST)Test.run('calendar');
	...
	function showSelectMenus(target) {
		...
		if ($.boxModel == true) {
			...
			seleMonth.height(sele.height()
					- (seleMonth.outerHeight() - seleMonth.height())
...
//if(__JS_TEST)Test.run('spinner');
	...
	function bindEvents(target) {
		var opts = $.data(target, 'spinner').options;
		var spinner = $.data(target, 'spinner').spinner;
		spinner.find('.spinner-arrow-up,.spinner-arrow-down')
				.unbind('.spinner');
		if (!opts.disabled) {
 ...
 */
(function($) {
//if(__JS_TEST)Test.run('numberspinner');
 ...
 */
(function($) {
//if(__JS_TEST)Test.run('timespinner');
	function create(target) {
	...
	function fixValue(target) {
		...
		var tt = [ formatNumber(time.getHours()),
 ...
 */
(function($) {
//if(__JS_TEST)Test.run('datagrid');
	...
	function fitGridSize(target) {
		...
		var innerHeader = gridHeader1.children('div.datagrid-header-inner')
				.show();
		...
		if ($.boxModel == true) {
			gridHeader1.height(hh
					- (gridHeader1.outerHeight() - gridHeader1.height()));
			gridHeader2.height(hh
					- (gridHeader2.outerHeight() - gridHeader2.height()));
		} else {
			gridHeader1.height(hh);
			gridHeader2.height(hh);
		}
		if (opts.height != 'auto') {
			var fixedHeight = height
					- gridView2.children('div.datagrid-header').outerHeight(
							true)
					- gridView2.children('div.datagrid-footer').outerHeight(
							true)
	...
	function getGridData(target) {
		...
		var fields = getColumnFields(target, true).concat(
				getColumnFields(target, false));
			...
			if (typeof opts.toolbar == 'string') {
				...
				for ( var i = 0; i < opts.toolbar.length; i++) {
					...
					if (btn == '-') {
						...
						var tool = $('<a href="javascript:void(0)" '
			...
			if (opts.onDblClickRow) {
				...
				function(e) {
					...
					if (opts.onRowContextMenu) {
						opts.onRowContextMenu.call(target, e, rowIndex,
								data.rows[rowIndex]);
					}
				});
	...
	function setProperties(target) {
		...
		gridView2.children('div.datagrid-body').unbind('.datagrid').bind(
				'scroll.datagrid',
				function() {
					gridView1.children('div.datagrid-body').scrollTop(
							$(this).scrollTop());
					gridView2.children('div.datagrid-header').scrollLeft(
							$(this).scrollLeft());
					gridView2.children('div.datagrid-footer').scrollLeft(
							$(this).scrollLeft());
				});
		function bindGridCellClick(gridCell, click) {
			gridCell.unbind('.datagrid');
			if (!click) {
				return;
			}
			gridCell.bind('click.datagrid',
					function(e) {
						...
						gridHeader.find('div.datagrid-cell').removeClass(
		...
		bindGridCellClick(gridHeader.find('div.datagrid-cell'), true);
		gridHeader
									...
									.resizable(
											{
												...
												onStopResize : function(e) {
													//greadHeader.css('cursor', '');
													var field = $(this)
															.parent().attr(
																	'field');
													var col = getColumnOption(
															target, field);
													col.width = $(this)
															.outerWidth();
													...
													fitColumns(target);
													setTimeout(function() {
														bindGridCellClick($(e.data.target),
																true);
													}, 0);
													var gridView2 = panel
		...
		gridView1.children('div.datagrid-header').find('div.datagrid-cell')
				.resizable(
						{
							onStopResize : function(e) {
								var field = $(this).parent().attr('field');
								...
								var gridView2 = panel
										.find('div.datagrid-view2');
								gridView2.find('div.datagrid-header')
										.scrollLeft(
												gridView2.find(
														'div.datagrid-body')
														.scrollLeft());
								...
								fitColumns(target);
								setTimeout(function() {
									bindGridCellClick($(e.data.target), true);
								}, 0);
								opts.onResizeColumn.call(target, field,
										col.width);
							}
						});
	};
	function fitColumns(target) {
		...
		var innerHeader = gridHeader2.children('div.datagrid-header-inner')
				.show();
		var fullWidth = gridHeader2.width() - gridHeader2.find('table').width()
				- opts.scrollbarSize;
	...
	function renderGrid(target, data) {
		...
		opts.view.render.call(opts.view, target, gridView2
				.children('div.datagrid-body'), false);
		opts.view.render.call(opts.view, target, gridView1.children(
	...
	function unselectAll(target) {
		...
		$.fn.prop ? checkbox.prop('checked', false) : checkbox.attr('checked',
				false);
		$('div.datagrid-body tr.datagrid-row-selected', panel).removeClass(
				'datagrid-row-selected');
		if (opts.idField) {
			for ( var rowIndex = 0; rowIndex < data.rows.length; rowIndex++) {
				selectedRows.removeById(opts.idField,
	...
	function selectRow(target, rowIndex) {
		...
		var tr = $('div.datagrid-body tr[datagrid-row-index=' + rowIndex + ']',
				panel);
		...
		if (top <= 0) {
			...
			if (top + tr.outerHeight() > gridBody.height() - 18) {
				gridBody.scrollTop(gridBody.scrollTop() + top
						+ tr.outerHeight() - gridBody.height() + 18);
			}
		}
	};
	...
	function validateRow(target, rowIndex) {
		var tr = $.data(target, 'datagrid').options.editConfig.getTr(target,
				rowIndex);
		if (!tr.hasClass('datagrid-row-editing')) {
			return true;
		}
	...
	var editors = {
		text : {
			...
			resize : function(target, width) {
				...
				if ($.boxModel == true) {
					editor
							.width(width
									- (editor.outerWidth() - editor.width()));
				} else {
					editor.width(width);
				}
			}
		...
		checkbox : {
			...
			setValue : function(target, value) {
				...
				$.fn.prop ? $(target).prop('checked', checked) : $(target)
	...
	var view = {
		...
		renderRow : function(target, fields, frozen, rowIndex, rowData) {
			...
			for ( var i = 0; i < fields.length; i++) {
				...
				if (col) {
					...
					if(typeof col.checkable != 'function'){
						col.checkable = function(){
							return true;
						}
					}
					if (col.checkbox ) {
						if(typeof col.checkable == 'function' && col.checkable.call(this,rowData,rowIndex)){
					...
					if (col.checkbox) {
						if(typeof col.checkable == 'function' && col.checkable.call(this,rowData,rowIndex)){
							...
							cc.push('<input type="checkbox" disabled="disabled"/>');
						}
					} else {
						if (col.formatter) {
							cc.push(col.formatter(rowData[field], rowData,rowIndex,field));
						} else {
		...
		refreshRow : function(target, rowIndex) {
			var row = {};
			var fields = $(target).datagrid('getColumnFields', true).concat(
		...
		updateRow : function(target, rowIndex, row) {
			...
			for ( var field in row) {
				...
				var col = $(target).datagrid('getColumnOption', field);
				if (col) {
					var style = col.styler ? col.styler(rows[rowIndex][field],
		...
		insertRow : function(target, rowIndex, row) {
			var opts = $.data(target, 'datagrid').options;
			var data = $.data(target, 'datagrid').data;
			var view = $(target).datagrid('getPanel').children(
					'div.datagrid-view');
			var gridView1 = view.children('div.datagrid-view1');
			var gridView2 = view.children('div.datagrid-view2');
			...
			for ( var i = data.rows.length - 1; i >= rowIndex; i--) {
				gridView2.children('div.datagrid-body').find(
						'tr[datagrid-row-index=' + i + ']').attr(
						'datagrid-row-index', i + 1);
				var tr = gridView1.children('div.datagrid-body').find(
						'tr[datagrid-row-index=' + i + ']').attr(
						'datagrid-row-index', i + 1);
				if (opts.rownumbers) {
			...
			var tr1 = '<tr datagrid-row-index="'
					+ rowIndex
					+ '">'
					+ this.renderRow.call(this, target, frozenFields, true,
							rowIndex, row) + '</tr>';
			var tr2 = '<tr datagrid-row-index="'
					+ rowIndex
					+ '">'
					+ this.renderRow.call(this, target, fields, false,
							rowIndex, row) + '</tr>';
			if (rowIndex >= data.rows.length) {
				var gridBody1 = gridView1.children('div.datagrid-body')
				...
				gridView1.children('div.datagrid-body').find(
						'tr[datagrid-row-index=' + (rowIndex + 1) + ']')
						.before(tr1);
				gridView2.children('div.datagrid-body').find(
						'tr[datagrid-row-index=' + (rowIndex + 1) + ']')
						.before(tr2);
		...
		deleteRow : function(target, rowIndex) {
			...
			var gridView2 = view.children('div.datagrid-view2');
			gridView1.children('div.datagrid-body').find(
					'tr[datagrid-row-index=' + rowIndex + ']').remove();
			gridView2.children('div.datagrid-body').find(
					'tr[datagrid-row-index=' + rowIndex + ']').remove();
		...
		onAfterRender : function(target) {
			var opts = $.data(target, 'datagrid').options;
			if (opts.showFooter) {
				var footer = $(target).datagrid('getPanel').find(
						'div.datagrid-footer');
				footer.find(
		...
		editConfig : {
			getTr : function(target, rowIndex) {
				return $(target).datagrid('getPanel').find(
						'div.datagrid-body tr[datagrid-row-index=' + rowIndex
								+ ']');
			},
 ...
 */
(function($) {
//if(__JS_TEST)Test.run('propertygrid');
	function init(target) {
		...
		$(target).datagrid('getPanel').find('div.datagrid-body').unbind(
					...
					$.fn.datagrid.defaults.view,
					{
						render : function(target, container, frozen) {
							...
							for ( var i = 0; i < groups.length; i++) {
								...
								if (!frozen) {
									html.push('<span>');
									html.push(opts.groupFormatter.call(target,
											group.fvalue, group.rows));
									html.push('</span>');
								}
								html.push('</td>');
								html.push('</tr>');
								...
								for ( var j = 0; j < group.rows.length; j++) {
									...
									var style = opts.rowStyler ? opts.rowStyler
											.call(target, rowIndex,
													group.rows[j]) : '';
									style = style ? 'style="' + style + '"'
											: '';
									html.push('<tr datagrid-row-index="'
											+ rowIndex + '" ' + cls + ' '
											+ style + '>');
									html.push(this.renderRow.call(this, target,
											fields, frozen, rowIndex,
											group.rows[j]));
									html.push('</tr>');
									rowIndex++;
								}
 ...
 */
(function($) {
//if(__JS_TEST)Test.run('treegrid');
						...
						onBeforeRender : function(target, nodeId, param) {
							...
							if (node) {
								...
								$.data(target, 'treegrid').data = $.data(
										target, 'treegrid').data.concat(param);
							}
							if (!opts.remoteSort) {
								this.sort(target, param);
							}
							this.treeNodes = param;
							this.treeLevel = $(target).treegrid('getLevel',
									nodeId);
						},
 ...
 */
(function($) {
//if(__JS_TEST)Test.run('combobox');
 ...
 */
(function($) {
	//if(__JS_TEST)Test.run('combotree');
	function setSize(target) {
	...
	function select(target) {
		...
		$(target).combo('setValues', vv).combo('setText',
				ss.join(opts.separator));
	};
	function setValues(target, values) {
		var opts = $.data(target, 'combotree').options;
		var combo = $.data(target, 'combotree').tree;
 ...
 */
(function($) {
//if(__JS_TEST)Test.run('combogrid');
	function create(target) {
	...
	function selectRow(target, step) {
		...
		if (rows.length) {
			index = grid.datagrid('getRowIndex',
					rows[rows.length - 1][opts.idField]);
			index += step;
			if (index < 0) {
				index = 0;
			}
	...
	function createBox(target) {
		...
		$(target).combo($.extend({}, opts, {
			onShowPanel : function() {
				...
				opts.onShowPanel.call(target);
				if ($.browser.msie) {
					state.calendar.bgIframe();// XXX for ie6
				}
			}
		}));
		...
		function createCalendar() {
			var panel = $(target).combo('panel');
			state.calendar = $('<div></div>').appendTo(panel).wrap(
			...
			$('<a href="javascript:void(0)" class="datebox-close"></a>').html(
					opts.closeText).appendTo(button);
...
(function($) {
	...
	//if(__JS_TEST)Test.run('datetimebox');
	function createBox(target) {
	...
	function setValue(target, value, remainText) {
		...
		var date = opts.parser(value);
		$(target).datetimebox('calendar')
				.calendar('moveTo', opts.parser(value));
		$(target).datetimebox('spinner')
				.timespinner('setValue', getTimeS(date));
		function getTimeS(date) {
			function formatNumber(value) {
				return (value < 10 ? '0' : '') + value;
			};
			var tt = [ formatNumber(date.getHours()),
					formatNumber(date.getMinutes()) ];
			if (opts.showSeconds) {
				tt.push(formatNumber(date.getSeconds()));
			}
			return tt.join($(target).datetimebox('spinner').timespinner(
					'options').separator);
		};
	};
